...
FCK.InitializeBehaviors = function () {
    ...
    this._ExecCheckCaret = function (evt) {
        ...
        var E = function () {
            ...
            if (!(I == 'a' || String(H.parentNode.contentEditable) == 'false' || (!(FCKListsLib.BlockElements[I] || FCKListsLib.NonEmptyBlockElements[I]) && B == 35)))
                return;
...
FCKXHtml._AppendNode = function (A, B) {
    ...
    switch (B.nodeType) {
        case 1:
            ...
            if (FCKBrowserInfo.IsGecko && B.nextSibling && (B.hasAttribute('_moz_editor_bogus_node') || B.getAttribute('type') == '_moz'))
                return false;
...
FCKStyle.prototype = {GetType: function () {
        ...
        while ((F = E.GetNextParagraph())) {
            ...
            if (K && !L) {
                ...
                FCKDomTools.MoveChildren(F, J);
            F.parentNode.insertBefore(J, F);
            FCKDomTools.RemoveNode(F);
        }
        ;
        for (var i = 0; i < H.length - 1; i++) {
            if (FCKDomTools.GetNextSourceElement(H[i], true, [], [], true) != H[i + 1])
                continue;
...
var FCKEnterKey = function (A, B, C, D) {
    ...
    E.SetKeystrokes([[13, 'Enter'], [SHIFT + 13, 'ShiftEnter'], [9, 'Tab'], [8, 'Backspace'], [CTRL + 8, 'CtrlBackspace'], [46, 'Delete']]);
    if (D > 0) {
        this.TabText = '';
        while (D-- > 0)
            this.TabText += '\xa0';
...
FCKTableHandler.MergeDown = function () {
    ...
    while (D && D.childNodes && D.childNodes.length > 0)
        E.appendChild(D.removeChild(D.firstChild));
    if (E.firstChild)
        E.insertBefore(FCKTools.GetElementDocument(D).createElement('br'), E.firstChild);
    B.appendChild(E);
    this._MarkCells([D], '_Replace');
    this._ReplaceCellsByMarker(C, '_Replace', B);
    this._InstallTableMap(C, B.parentNode.parentNode);
...
FCKFormatBlockCommand.prototype = {Name: 'FormatBlock', Execute: FCKStyleCommand.prototype.Execute, GetState: function () {
            ...
            if (FCKConfig.EnterMode.IEquals('br')) {
                while (R.length) {
                    ...
                    if (S.nodeName.IEquals('div')) {
                        ...
                        if (W && Y)
                            M.appendChild(FCKTools.GetElementDocument(S).createElement('br'));
...
var FCKDialog = (function () {
    ...
    var I = function (element) {
        element.style.cssText = 'margin:0;padding:0;border:0;background-color:transparent;background-image:none;';
    };
