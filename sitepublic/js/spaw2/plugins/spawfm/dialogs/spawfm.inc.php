...
if ($curr_type) {
    ...
    foreach ($dirs as $key => $val) {
        if (empty($val['params']['allowed_filetypes']) or ( is_string($val['params']['allowed_filetypes']) and $val['params']['allowed_filetypes'] != 'any' and $val['params']['allowed_filetypes'] != $curr_type) or ( is_array($val['params']['allowed_filetypes']) and ! in_array($curr_type, $val['params']['allowed_filetypes']) and ! in_array('any', $val['params']['allowed_filetypes']))) {
            unset($dirs[$key]);
            if ($val['dir'] == $curr_dir) {
                $curr_dir = false;
...
if ($del_file = SpawVars::getPostVar('delete_file')) {
    ...
    if (file_exists($spawfm->getCurrentFsDir() . $del_file)) {
        if (is_dir($spawfm->getCurrentFsDir() . $del_file)) {
            if (!$spawfm->getCurrentDirSetting('recursive') or ! $spawfm->getCurrentDirSetting('allow_modify_subdirectories')) {
...
?>
                        ...
                        <td valign="middle" nowrap="nowrap">
                                ...
                                if (!$spawfm->getCurrentDirSetting('recursive') or ! $spawfm->getCurrentDirSetting('allow_create_subdirectories')) {
