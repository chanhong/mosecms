...
var TinyMCE_TablePlugin = {getInfo: function() {
        return{longname: 'Tables', author: 'Moxiecode Systems AB', authorurl: 'http://tinymce.moxiecode.com', infourl: 'http://tinymce.moxiecode.com/tinymce/docs/plugin_table.html', version: tinyMCE.majorVersion + "." + tinyMCE.minorVersion}
                ...
                if (tableElm && trElm) {
                    switch (command) {
                        ...
                        case"mceTableInsertRowBefore":
                            ...
                            inst.selection.selectNode(getCell(grid, cpos.rowindex + 1, cpos.cellindex), tinyMCE.isGecko, true);
                            break;
                        case"mceTableInsertRowAfter":
                            if (!trElm || !tdElm)
                                return true;
                            ...
                            grid = getTableGrid(tableElm);
                            inst.selection.selectNode(getCell(grid, cpos.rowindex, cpos.cellindex), tinyMCE.isGecko, true);
                            break;
                        case"mceTableDeleteRow":
                            if (!trElm || !tdElm)
                                return true;
                            ...
                            var cpos = getCellPos(grid, tdElm);
                            if (grid.length == 1) {
                                tableElm = tinyMCE.getParentElement(tableElm, "table");
                        ...
                        case"mceTableInsertColBefore":
                            ...
                            grid = getTableGrid(tableElm);
                            inst.selection.selectNode(getCell(grid, cpos.rowindex, cpos.cellindex + 1), tinyMCE.isGecko, true);
                            break;
                        case"mceTableInsertColAfter":
                            if (!trElm || !tdElm)
                                return true;
